...
func (jwt *JWT) ParserToken(ctx *gin.Context) (*JWTCustomClaims, error) {
	tokenString, parseErr := jwt.getTokenFromHeader(ctx)
	if parseErr != nil {
		return nil, parseErr
	}
...
// createToken 创建 Token，内部使用，外部需调用 IssueToken
func (jwt *JWT) createToken(claims JWTCustomClaims) (string, error) {
	// 使用 HS256 算法，进行 token 生成
...
func (jwt *JWT) getTokenFromHeader(ctx *gin.Context) (string, error) {
